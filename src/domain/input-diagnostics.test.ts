...
import { processStickInput } from "./input-diagnostics";
...
describe("stick processing", () => {
  it("keeps direction while applying the radial curve", () => {
    const result = processStickInput({ x: 0.5, y: 0.5 }, curve, false);
    expect(result.x).toBeCloseTo(result.y);
    expect(Math.hypot(result.x, result.y)).toBeCloseTo(Math.hypot(0.5, 0.5));
  });

  it("limits processed output to a circle when requested", () => {
    const result = processStickInput({ x: 1.03, y: 1.03 }, curve, true);
